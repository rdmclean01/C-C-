...
void queue_increment(queue_index_t* index, queue_t* q)
{
    // Check if we're at the top of the queue and must wrap around
    if (*index == q->size-1)    // If the index is equal to the queue size minus 1
        *index = 0;             // Set the index to 0 (the bottom of the queue)
    else                        // The index was not at the top of the queue
...
{
    queue_index_t indexIn_dummy = q->indexIn;           // Create dummy indexIn to test increment
    queue_increment(&indexIn_dummy, q);                 // Increment the dummy IndexIn
    if (indexIn_dummy == q->indexOut){                  // If the incremented dummy indexIn is equal to indexOut
        return true;
    }
    else                                                // If the incremented dummy indexIn was not equal to the indexOut
        return false;
}
...
{
    return (q->indexIn == q->indexOut); // Is empty if the indexIn is the same as indexOut
}
...
void queue_push(queue_t* q, queue_data_t value)
{
    if (!queue_full(q))
    {
        q->underflowFlag = false;                       // Reset the underflow flag
        q->overflowFlag = false;                        // Reset the overflow flag
        q->data[q->indexIn] = value;                    // Set the data at the indexIn to the given variable "value"
        q->elementCount++;                              // Increment element count
        queue_increment(&(q->indexIn), q);              // Increment the indexIn
    }
    else
    {
        q->overflowFlag = true;                         // Set the overflow flag
        printf("queue_push: failed, queue full\n\r");   // Print the overflow error message
    }
}
...
{
    queue_data_t value_to_return = NULL;                // Danger initialize return variable
    if (!queue_empty(q))
    {
        q->underflowFlag = false;                       // Reset the underflow flag
        q->overflowFlag = false;                        // Reset the overflow flag
        value_to_return = q->data[q->indexOut];         // Set the return value to the data at indexOut
        q->elementCount--;                              // Decrement element count
        queue_increment(&(q->indexOut), q);             // Increment the indexOut
    }
    else
    {
        q->underflowFlag = true;                        // Set the underflow flag
        printf("queue_pop: failed, queue empty\n\r");   // Print underflow error message
...
void queue_overwritePush(queue_t* q, queue_data_t value)
{
    ...
    {
        queue_pop(q);               // Pop the oldest value (throwing it away)
        queue_push(q,value);        // Push the new value onto the queue
    }
    else                            // Otherwise the queue is not full
    {                               // No need to pop oldest value to throw it away
        queue_push(q,value);        // Push the new value onto the queue
    }
}
...
{
    queue_index_t desired_index = q->indexOut + index; // actual index considering indexOut location offset

    if (index >= q->elementCount)       // If index is above indexOut
    {
        printf("queue_readElementAt: no element at index: %d, queue length: %d\n\r", index, q->elementCount);
        return NULL;                        // Danger: return null on error?
    }

    else if (q->size > (desired_index)){              // If the queue size is greater than the desired index -> no wrap-around needed
        return q->data[desired_index];           // Return the data at the index + indexOut offset
    }
    else                                        // Otherwise the desired index is greater than or equal to the size -> need wrap-around
        return q->data[desired_index - q->size];// Return the data at the wrap-around offset index
}
...
{
    queue_index_t loopIndex;
    for (loopIndex = 0; loopIndex < q->elementCount; loopIndex++)
    {
        printf("Index: %l\t Value: %l\n\r", loopIndex, queue_readElementAt(q,loopIndex));
    }
}
